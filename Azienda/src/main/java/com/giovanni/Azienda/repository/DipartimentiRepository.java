...
public interface DipartimentiRepository extends JpaRepository<Dipartimenti, Integer>{
	...
	public Optional<Dipartimenti> findById(Integer idRep);
	
	@Query(value = "SELECT reparti.nome AS reparti, COUNT(*) AS totale_impiegati FROM reparti INNER JOIN impiegati  ON "
			+ "reparti.id_rep = impiegati.id_rep GROUP BY reparti.nome", nativeQuery = true)
